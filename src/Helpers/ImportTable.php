...
class ImportTable extends \WP_List_Table {
	...
	public function __construct() {
		...
		$this->import_log = \ST\Lms\fetch_import_data();
	...
	protected function get_views() {
		...
		$cnt_class = 'count';
		$status    = \ST\Lms\import_job_status();
		$cnt       = array_count_values( array_column( \ST\Lms\fetch_import_data( 0, true ), 'import_status' ) );
	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'type':
				$import  = \ST\Lms\import_post_type();
				$db_type = $item['import_type'];
				$type    = array_key_exists( $db_type, $import ) ? ucfirst( str_replace( 'stlms_', '', $import[ $db_type ] ) ) : '';
				return $type;
			case 'progress':
